...
| Nhóm thuật toán                     | Thuật toán              | Độ phức tạp thời gian             | Độ phức tạp bộ nhớ       | Ưu điểm                                       | Nhược điểm                                    | Hiệu quả trên 8-Puzzle             | Ghi chú                              |
|-----------------------------------|------------------------|---------------------------------|--------------------------|-----------------------------------------------|-----------------------------------------------|------------------------------------|-------------------------------------|
| 1. Tìm kiếm trong môi trường không xác định | And-Or Search          | Có thể lên đến O(b^d) hoặc hơn  | Rất cao                  | Xử lý môi trường không chắc chắn, chính sách đảm bảo | Không gian trạng thái rất lớn, tính toán phức tạp | Ít phù hợp với 8-Puzzle chuẩn       | Dùng khi môi trường có rủi ro, quan sát hạn chế |
|                                   | Belief State Search     | Cấp số nhân với kích thước belief states | Cao                      | Quản lý tập trạng thái tin tưởng trong môi trường không hoàn chỉnh | Không gian trạng thái mở rộng, phức tạp       | Khó áp dụng trực tiếp               | Phù hợp biến thể 8-Puzzle mở rộng   |
|                                   | Searching with Partial Observation | Tương tự hoặc cao hơn belief state search | Cao                      | Xử lý POMDP, quan sát hạn chế                  | Tính toán phức tạp, khó mở rộng                | Ít ứng dụng trực tiếp               | Mở rộng thực tế                    |
| 2. Tìm kiếm cục bộ (Local Search) | Simple Hill Climbing    | O(k * b), k là số bước           | O(b)                     | Dễ cài đặt, nhanh                              | Mắc kẹt cực trị cục bộ, không đảm bảo lời giải | Tìm lời giải nhanh, không tối ưu   | Phù hợp không gian lớn, lời giải gần đúng |
|                                   | Steepest-Ascent Hill Climbing | O(k * b)                      | O(b)                     | Tìm trạng thái lân cận tốt nhất                | Tốn thời gian so sánh, vẫn mắc kẹt cực trị      | Hiệu quả hơn Simple Hill Climbing  |                                   |
|                                   | Stochastic Hill Climbing | O(k * m), m ≤ b trạng thái tốt hơn | O(m)                     | Giảm mắc kẹt cực trị cục bộ nhờ tính ngẫu nhiên | Không ổn định, có thể chậm                      | Tăng khả năng tìm lời giải khả thi |                                   |
|                                   | Simulated Annealing     | O(k * b), k lớn do làm mát dần   | O(b)                     | Thoát cực trị cục bộ, tìm lời giải gần tối ưu  | Cần tinh chỉnh tham số làm mát                 | Hiệu quả cho bài toán lớn          |                                   |
|                                   | Local Beam Search       | O(k * b * k), k trạng thái giữ   | O(k * b)                 | Đa dạng hóa trạng thái tìm kiếm                | Cần bộ nhớ lớn hơn hill climbing                | Tăng khả năng tìm lời giải tối ưu  |                                   |
|                                   | Genetic Algorithm       | O(g * p * c), g: số thế hệ, p: kích thước quần thể | O(p)                     | Khám phá đa hướng, phù hợp không gian lớn      | Không đảm bảo tối ưu, khó điều chỉnh tham số   | Tìm lời giải gần đúng              | Phù hợp mở rộng                   |
| 3. Học củng cố (Reinforcement Learning) | Q-Learning              | O(t * |S| * |A|), t: số tập học  | Phụ thuộc số trạng thái và hành động | Không cần biết mô hình môi trường               | Đòi hỏi thời gian và tài nguyên lớn | Tiềm năng cho bài toán phức tạp     |
| 4. Tìm kiếm không thông tin (Uninformed Search) | BFS                    | O(b^d), d là độ sâu lời giải     | O(b^d)                   | Đảm bảo tìm lời giải tối ưu                     | Tốn bộ nhớ lớn                                  | Hiệu quả với 8-Puzzle nhỏ          | Hạn chế không gian lớn             |
|                                   | DFS                    | O(b^d)                          | O(d)                     | Tiết kiệm bộ nhớ                               | Không đảm bảo lời giải tối ưu, dễ vòng lặp     | Ít hiệu quả                       | Cần giới hạn độ sâu                |
|                                   | UCS                    | O(b^d)                          | O(b^d)                   | Tối ưu chi phí lời giải                         | Tốn bộ nhớ, chậm                                | Hiệu quả khi chi phí bước khác nhau |                                   |
|                                   | IDS                    | O(b^d)                          | O(d)                     | Kết hợp ưu điểm BFS và DFS, tiết kiệm bộ nhớ   | Tốn thời gian hơn BFS                            | Hiệu quả tốt với 8-Puzzle giới hạn |                                   |
| 5. Tìm kiếm có thông tin (Informed Search) | A*                     | O(b^{1+ε}), ε phụ thuộc heuristic | O(b^{1+ε})               | Tối ưu lời giải, tốc độ cao                      | Tốn bộ nhớ, phụ thuộc heuristic                | Phương pháp chuẩn, hiệu quả nhất  |                                   |
|                                   | IDA*                   | O(b^d)                         | O(d)                     | Tiết kiệm bộ nhớ hơn A*, giữ tính tối ưu        | Thời gian có thể lâu hơn A*                      | Thích hợp khi bộ nhớ hạn chế      |                                   |
|                                   | Greedy Best-First Search | O(b^m), m là chiều sâu tối thiểu | O(b^m)                   | Nhanh, đơn giản                                | Không đảm bảo tối ưu, dễ mắc kẹt                 | Tốt cho lời giải nhanh            | Không tối ưu                     |
