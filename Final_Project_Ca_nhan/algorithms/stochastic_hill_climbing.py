...
class StochasticHillClimbingAlgorithm:
    ...
    def StochasticHillClimbing(self, trang_thai_dau_vao):
        ...
        while True:
            ...
            for con in cac_con:
                ...
                if con_so not in tap_da_tham:
                    ...
                    if heuristic < heuristic_hien_tai:
                        cac_con_tot_hon.append(con_so)

            if not cac_con_tot_hon:
                self.counter = dem
                self.depth = do_sau
                self.time_taken = float(time.time() - thoi_gian_bat_dau)
                return [], 0, self.counter, self.depth, self.time_taken, self.max_visited_size

            trang_thai_tiep = random.choice(cac_con_tot_hon)
